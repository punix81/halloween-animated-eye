...
- permet de televerser un nouveau GIF depuis un telephone ;
- permet de supprimer un GIF non actif depuis l interface web ;
...
5. Utiliser le formulaire pour televerser un nouveau GIF vers `/gifs`.

6. Utiliser `Supprimer` pour effacer un GIF non actif.
...
`POST /api/upload` utilise `multipart/form-data` avec un champ fichier `gif`. Le sketch ferme l animation courante, ecrit progressivement les chunks recus dans `/gifs/.upload.tmp`, verifie le fichier, puis renomme le temporaire vers le nom definitif.

Validations upload :

- taille superieure a zero ;
- limite configurable `MAX_UPLOAD_SIZE_BYTES`, par defaut 10 Mo ;
...
`POST /api/delete` attend un parametre `path`. La suppression est interdite pour le GIF actuellement utilise et pour le GIF de secours `/eye.gif`. La lecture est suspendue pendant l operation SD, puis reprise ensuite.
...
- `SD inaccessible` : l upload, la suppression et la lecture depuis `/gifs` sont refuses, mais le serveur web reste disponible.
- `Nom de fichier GIF invalide` : le nom contient un chemin, `..`, `\`, un guillemet, un caractere de controle, commence par `.`, ou ne finit pas par `.gif`.
- `Fichier trop grand` : la taille recue depasse `MAX_UPLOAD_SIZE_BYTES`.
- `Fichier vide` : aucun octet utile n a ete recu.
- `Signature GIF non compatible` : les six premiers octets ne sont ni `GIF87a` ni `GIF89a`.
- `Manque d espace pendant l ecriture` : l ecriture d un chunk sur la microSD n a pas ecrit tous les octets.
- `Un GIF porte deja ce nom` : l upload refuse d ecraser un fichier existant.
- `Suppression du GIF actif ou de secours interdite` : securite pour eviter de couper l animation en cours ou de perdre `/eye.gif`.
...
11. Depuis le telephone, televerser un nouveau GIF de moins de 10 Mo avec le formulaire.

12. Verifier que la barre de progression avance pendant le transfert.

13. Verifier que le nouveau GIF apparait automatiquement dans la liste.

14. Cliquer sur `Lire` pour le nouveau GIF.

15. Supprimer un GIF non actif et verifier qu il disparait de la liste.

16. Verifier que la suppression du GIF actif est refusee.

17. Tester le bouton lecture/pause sans redemarrer l'ESP32-S2.

18. Tester un GIF invalide ou trop grand : la page et le moniteur serie doivent afficher une erreur, puis le dernier GIF valide doit reprendre.
...
- Les uploads sont ecrits chunk par chunk vers `/gifs/.upload.tmp`, puis renommes apres validation.
- Aucune lecture GIF n est maintenue pendant une ecriture ou suppression SD.
